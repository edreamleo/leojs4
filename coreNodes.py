# -*- coding: utf-8 -*-
#@+leo-ver=5-thin
#@+node:ekr.20201227063438.1: * @file coreNodes.py
#@@first
#@@path c:/vs-code-devel/leojs4/
"""Leo's fundamental data classes, for leoInteg."""

from leo.core import leoGlobals as g

#@+others
#@+node:ekr.20201227063438.3: ** class NodeIndices
class NodeIndices:
    ...
    #@+others
    #@+node:ekr.20201227064554.1: *3* ni._get_time (new)
    def _get_time(self):
        ...
        # return time.strftime(
            ...
            # time.localtime())
    #@+node:ekr.20201227063438.5: *3* ni.check_gnx
    def check_gnx(self, c, gnx, v):
        ...
        if v2 and v2 != v:
            g.error(
                ...
                f"          v: {v}\n"
                f"         v2: {v2}")
    #@+node:ekr.20201227063438.6: *3* ni.compute_last_index
            ...
            if gnx:
                ...
                if t == ni.timeString and n is not None:
                    try:
                        ...
                        g.es_exception()
                        self.lastIndex += 1
    #@+node:ekr.20201227063438.7: *3* ni.computeNewIndex
    def computeNewIndex(self):
        ...
        return gnx
    #@+node:ekr.20201227063438.8: *3* ni.get/setDefaultId
    ...
    #@+node:ekr.20201227063438.9: *3* ni.getNewIndex
    def getNewIndex(self, v, cached=False):
        ...
        **Important**: the method must allocate a new gnx even if v.fileIndex exists.
        ...
        fc.gnxDict[gnx] = v
        return gnx
    #@+node:ekr.20201227063438.10: *3* ni.new_vnode_helper
    ...
    #@+node:ekr.20201227063438.11: *3* ni.scanGnx
    def scanGnx(self, s, i=0):
        ...
        return theId, t, n
    #@+node:ekr.20201227063438.12: *3* ni.setTimeStamp
    def setTimeStamp(self):
        """Set the timestamp string to be used by getNewIndex until further notice"""
        self.timeString = self._get_time()
    #@+node:ekr.20201227063438.13: *3* ni.tupleToString
    ...
    #@+node:ekr.20201227063438.14: *3* ni.update
    def update(self):
        ...
        if self.timeString == t_s:
            ...
            self.timeString = t_s
        return t_s
    #@+node:ekr.20201227063438.15: *3* ni.updateLastIndex
    ...
    #@-others
#@+node:ekr.20201227063438.16: ** class Position
#@+<< about the position class >>
#@+node:ekr.20201227063438.17: *3* << about the position class >>
#@@language rest
#@+at
...
class Position:
    #@+others
    #@+node:ekr.20201227063438.18: *3*  p.ctor & other special methods...
    #@+node:ekr.20201227063438.19: *4*  p.__init__
    def __init__(self, v, childIndex=0, stack=None):
        ...
        if stack:
            ...
            self.stack = []
    #@+node:ekr.20201227063438.20: *4* p.__eq__ & __ne__
    def __eq__(self, p2):
    ...
    #@+node:ekr.20201227063438.21: *4* p.__ge__ & __le__& __lt__
    ...
    #@+node:ekr.20201227063438.22: *4* p.__gt__
    def __gt__(self, other):
        ...
        return x2 >= x1
    #@+node:ekr.20201227063438.23: *4* p.__nonzero__ & __bool__
    def __bool__(self):
        """
    ...
    #@+node:ekr.20201227063438.24: *4* p.__str__ and p.__repr__
    def __str__(self):
        p = self
        if p.v:
            return (
    ...
    __repr__ = __str__
    #@+node:ekr.20201227063438.25: *4* p.archivedPosition
    def archivedPosition(self, root_p=None):
        ...
        aList.reverse()
        return aList
    #@+node:ekr.20201227063438.26: *4* p.dump
    ...
    #@+node:ekr.20201227063438.27: *4* p.key & p.sort_key & __hash__
    def key(self):
        p = self
    ...
    __hash__ = None
    #@+node:ekr.20201227063438.28: *3* p.File Conversion
    #@+at
    # - convertTreeToString and moreHead can't be VNode methods because they uses level().
    # - moreBody could be anywhere: it may as well be a postion method.
    #@+node:ekr.20201227063438.29: *4* p.convertTreeToString
    def convertTreeToString(self):
        ...
        for p in p.self_and_subtree(copy=False):
            ...
            if body:
                array.append(body + '\n')
        return ''.join(array)
    #@+node:ekr.20201227063438.30: *4* p.moreHead
    ...
    #@+node:ekr.20201227063438.31: *4* p.moreBody
    #@@language rest
    #@+at
    #     + test line
    #     - test line
    ...
    def moreBody(self):
        ...
        for s in lines:
            ...
            array.append(s)
        return '\n'.join(array)
    #@+node:ekr.20201227063438.32: *3* p.generators
    #@+node:ekr.20201227063438.33: *4* p.children
    ...
    #@+node:ekr.20201227063438.34: *4* p.following_siblings
    ...
    #@+node:ekr.20201227063438.35: *4* p.nearest_roots
    def nearest_roots(self, copy=True, predicate=None):
        ...
        while p and p != after:
            if predicate(p):
                ...
                p.moveToThreadNext()
    #@+node:ekr.20201227063438.36: *4* p.nearest_unique_roots (aka p.nearest)
    ...
    #@+node:ekr.20201227063438.37: *4* p.nodes
    def nodes(self):
    ...
    tnodes_iter = nodes
    vnodes_iter = nodes
    #@+node:ekr.20201227063438.38: *4* p.parents
    ...
    #@+node:ekr.20201227063438.39: *4* p.self_and_parents
    ...
    #@+node:ekr.20201227063438.40: *4* p.self_and_siblings
    ...
    #@+node:ekr.20201227063438.41: *4* p.self_and_subtree
    ...
    #@+node:ekr.20201227063438.42: *4* p.subtree
    ...
    #@+node:ekr.20201227063438.43: *4* p.unique_nodes
    ...
    #@+node:ekr.20201227063438.44: *4* p.unique_subtree
    ...
    #@+node:ekr.20201227063438.45: *3* p.Getters
    #@+node:ekr.20201227063438.46: *4* p.VNode proxies
    #@+node:ekr.20201227063438.47: *5* p.Comparisons
    ...
    #@+node:ekr.20201227063438.48: *5* p.Headline & body strings
    def bodyString(self):
    ...
    #@+node:ekr.20201227063438.49: *5* p.Status bits
    ...
    #@+node:ekr.20201227063438.50: *4* p.children & parents
    #@+node:ekr.20201227063438.51: *5* p.childIndex
    # This used to be time-critical code.

    def childIndex(self):
        p = self
        return p._childIndex
    #@+node:ekr.20201227063438.52: *5* p.directParents
    def directParents(self):
        return self.v.directParents()
    #@+node:ekr.20201227063438.53: *5* p.hasChildren & p.numberOfChildren
    ...
    #@+node:ekr.20201227063438.54: *4* p.getX & VNode compatibility traversal routines
    ...
    hasVisBack = visBack
    hasVisNext = visNext
    #@+node:ekr.20201227063438.55: *4* p.get_UNL
    def get_UNL(
        self,
        with_file=True,
        with_proto=False,
        ...
        return UNL
    #@+node:ekr.20201227063438.56: *4* p.hasBack/Next/Parent/ThreadBack
    def hasBack(self):
        p = self
    ...
    #@+node:ekr.20201227063438.57: *5* hasThreadNext (the only complex hasX method)
    def hasThreadNext(self):
        ...
        while n >= 0:
            ...
            n -= 1
        return False
    #@+node:ekr.20201227063438.58: *4* p.findRootPosition
    def findRootPosition(self):
        # 2011/02/25: always use c.rootPosition
        p = self
        c = p.v.context
        return c.rootPosition()
    #@+node:ekr.20201227063438.59: *4* p.isAncestorOf
    def isAncestorOf(self, p2):
        ...
        for z in p2.stack:
            ...
            if parent_v == p.v and parent_childIndex == p._childIndex:
                return True
        return False
    #@+node:ekr.20201227063438.60: *4* p.isCloned
    def isCloned(self):
        p = self
        return p.v.isCloned()
    #@+node:ekr.20201227063438.61: *4* p.isRoot
    def isRoot(self):
        p = self
        return not p.hasParent() and not p.hasBack()
    #@+node:ekr.20201227063438.62: *4* p.isVisible (slow)
    def isVisible(self, c):
        ...
        for root in c.rootPosition().self_and_siblings(copy=False):
            if root == p or root.isAncestorOf(p):
                return visible(p)
        return False
    #@+node:ekr.20201227063438.63: *4* p.level & simpleLevel
    def level(self):
    ...
    simpleLevel = level
    #@+node:ekr.20201227063438.64: *4* p.positionAfterDeletedTree
    ...
    #@+node:ekr.20201227063438.65: *4* p.textOffset
    def textOffset(self):
        """
    ...
    #@+node:ekr.20201227063438.66: *3* p.isOutsideAtFileTree
    def isOutsideAnyAtFileTree(self):
        ...
        for parent in p.self_and_parents(copy=False):
            if parent.isAnyAtFileNode():
                return False
        return True
    #@+node:ekr.20201227063438.67: *3* p.Low level methods
    # These methods are only for the use of low-level code
    # in leoNodes.py, leoFileCommands.py and leoUndo.py.
    #@+node:ekr.20201227063438.68: *4* p._adjustPositionBeforeUnlink
    def _adjustPositionBeforeUnlink(self, p2):
        ...
        if changed:
            p.stack = stack
    #@+node:ekr.20201227063438.69: *4* p._linkAfter
    ...
    #@+node:ekr.20201227063438.70: *4* p._linkCopiedAfter
    ...
    #@+node:ekr.20201227063438.71: *4* p._linkAsNthChild
    def _linkAsNthChild(self, parent, n):
        ...
        child = p.v
        child._addLink(n, parent_v)
    #@+node:ekr.20201227063438.72: *4* p._linkCopiedAsNthChild
    def _linkCopiedAsNthChild(self, parent, n):
        ...
        child = p.v
        child._addCopiedLink(n, parent_v)
    #@+node:ekr.20201227063438.73: *4* p._linkAsRoot (changed)
    def _linkAsRoot(self):
        ...
        return p
    #@+node:ekr.20201227063438.74: *4* p._parentVnode
    def _parentVnode(self):
        ...
        return None
    #@+node:ekr.20201227063438.75: *4* p._relinkAsCloneOf
    ...
    #@+node:ekr.20201227063438.76: *4* p._unlink
    def _unlink(self):
    ...
    #@+node:ekr.20201227063438.77: *5* p.badUnlink
    ...
    #@+node:ekr.20201227063438.78: *3* p.moveToX
    ...
    #@+node:ekr.20201227063438.79: *4* p.moveToBack
    def moveToBack(self):
        ...
        if parent_v and p.v and 0 < n <= len(parent_v.children):
            ...
            p.v = None
        return p
    #@+node:ekr.20201227063438.80: *4* p.moveToFirstChild
    def moveToFirstChild(self):
        ...
        if p.v and p.v.children:
            ...
            p.v = None
        return p
    #@+node:ekr.20201227063438.81: *4* p.moveToLastChild
    def moveToLastChild(self):
        ...
        if p.v and p.v.children:
            ...
            p.v = None
        return p
    #@+node:ekr.20201227063438.82: *4* p.moveToLastNode
    def moveToLastNode(self):
        ...
        while p.hasChildren():
            p.moveToLastChild()
        return p
    #@+node:ekr.20201227063438.83: *4* p.moveToNext
    def moveToNext(self):
        ...
        if p.v and parent_v and len(parent_v.children) > n + 1:
            ...
            p.v = None
        return p
    #@+node:ekr.20201227063438.84: *4* p.moveToNodeAfterTree
    def moveToNodeAfterTree(self):
        ...
        while p:
            ...
            p.moveToParent()
        return p
    #@+node:ekr.20201227063438.85: *4* p.moveToNthChild
    def moveToNthChild(self, n):
        ...
        if p.v and len(p.v.children) > n:
            ...
            p.v = None
        return p
    #@+node:ekr.20201227063438.86: *4* p.moveToParent
    def moveToParent(self):
        ...
        if p.v and p.stack:
            ...
            p.v = None
        return p
    #@+node:ekr.20201227063438.87: *4* p.moveToThreadBack
    def moveToThreadBack(self):
        ...
        if p.hasBack():
            ...
            p.moveToParent()
        return p
    #@+node:ekr.20201227063438.88: *4* p.moveToThreadNext
    def moveToThreadNext(self):
        ...
        if p.v:
            if p.v.children:
                ...
                # not found.
        return p
    #@+node:ekr.20201227063438.89: *4* p.moveToVisBack & helper
        ...
        while p:
            ...
            if p:
                ...
                if p.isVisible(c):
                    return p
        return p
    #@+node:ekr.20201227063438.90: *5* checkVisBackLimit
    def checkVisBackLimit(self, limit, limitIsVisible, p):
        ...
        if limit.isAncestorOf(p):
            return False, None
        return True, None
    #@+node:ekr.20201227063438.91: *4* p.moveToVisNext & helper
        ...
        while p:
            ...
            if p:
                ...
                if p.isVisible(c):
                    return p
        return p
    #@+node:ekr.20201227063438.92: *5* checkVisNextLimit
    ...
    #@+node:ekr.20201227063438.93: *4* p.safeMoveToThreadNext
    def safeMoveToThreadNext(self):
        ...
        if p.v:
            ...
            if child_v:
                ...
                # not found.
        return p
    #@+node:ekr.20201227063438.94: *5* p.checkChild
    #@+node:ekr.20201227063438.95: *3* p.Moving, Inserting, Deleting, Cloning, Sorting
    #@+node:ekr.20201227063438.96: *4* p.clone
    def clone(self):
        ...
        return p2
    #@+node:ekr.20201227063438.97: *4* p.copy
    def copy(self):
        """"Return an independent copy of a position."""
        return Position(self.v, self._childIndex, self.stack)
    #@+node:ekr.20201227063438.98: *4* p.copyTreeAfter, copyTreeTo
    ...
    #@+node:ekr.20201227063438.99: *4* p.copyWithNewVnodes
    ...
    #@+node:ekr.20201227063438.100: *4* p.createNodeHierarchy
    ...
    #@+node:ekr.20201227063438.101: *4* p.deleteAllChildren
    ...
    #@+node:ekr.20201227063438.102: *4* p.doDelete
    def doDelete(self, newNode=None):
        ...
        while sib.hasNext():
            ...
            if sib == newNode:
                ...
                break
        p._unlink()
    #@+node:ekr.20201227063438.103: *4* p.insertAfter
    def insertAfter(self):
        ...
        p2._linkAfter(p)
        return p2
    #@+node:ekr.20201227063438.104: *4* p.insertAsLastChild
    ...
    #@+node:ekr.20201227063438.105: *4* p.insertAsNthChild
    def insertAsNthChild(self, n):
        ...
        return p2
    #@+node:ekr.20201227063438.106: *4* p.insertBefore
    def insertBefore(self):
        ...
        if p.hasBack():
            ...
            p.moveToRoot()
        return p
    #@+node:ekr.20201227063438.107: *4* p.invalidOutline
    ...
    #@+node:ekr.20201227063438.108: *4* p.moveAfter
    def moveAfter(self, a):
        ...
        p._linkAfter(a)
        return p
    #@+node:ekr.20201227063438.109: *4* p.moveToFirst/LastChildOf
    ...
    #@+node:ekr.20201227063438.110: *4* p.moveToNthChildOf
    def moveToNthChildOf(self, parent, n):
        ...
        return p
    #@+node:ekr.20201227063438.111: *4* p.moveToRoot (changed)
    def moveToRoot(self):
        ...
        p._linkAsRoot()
        return p
    #@+node:ekr.20201227063438.112: *4* p.promote
    def promote(self):
    ...
    #@+node:ekr.20201227063438.113: *4* p.validateOutlineWithParent
    ...
    def validateOutlineWithParent(self, pv):
        ...
        #@+node:ekr.20201227063438.114: *5* << validate parent ivar >>
        if parent != pv:
            p.invalidOutline("Invalid parent link: " + repr(parent))
        #@-<< validate parent ivar >>
        #@+<< validate childIndex ivar >>
        #@+node:ekr.20201227063438.115: *5* << validate childIndex ivar >>
        if pv:
            if childIndex < 0:
        ...
        #@-<< validate childIndex ivar >>
        #@+<< validate x ivar >>
        #@+node:ekr.20201227063438.116: *5* << validate x ivar >>
        if not p.v and pv:
        ...
        return result
    #@+node:ekr.20201227063438.117: *3* p.Properties
    #@+node:ekr.20201227063438.118: *4* p.b property
    def __get_b(self):
    ...
    #@+node:ekr.20201227063438.119: *4* p.h property
    def __get_h(self):
        p = self
        return p.headString()
    ...
    #@+node:ekr.20201227063438.120: *4* p.gnx property
    def __get_gnx(self):
        p = self
    ...
    #@+node:ekr.20201227063438.122: *4* p.nosentinels property
    ...
    #@+node:ekr.20201227063438.123: *4* p.u Property
    def __get_u(self):
        p = self
        return p.v.u
    ...
    u = property(
        __get_u, __set_u,
        doc="p.u property")
    #@+node:ekr.20201227063438.124: *3* p.Setters
    #@+node:ekr.20201227063438.125: *4* p.VNode proxies
    #@+node:ekr.20201227063438.126: *5* p.contract/expand/isExpanded
    def contract(self):
    ...
    #@+node:ekr.20201227063438.127: *5* p.Status bits
    ...
    #@+node:ekr.20201227063438.128: *5* p.computeIcon & p.setIcon
    def computeIcon(self):
        return self.v.computeIcon()

    def setIcon(self):
        pass  # Compatibility routine for old scripts
    #@+node:ekr.20201227063438.129: *5* p.setSelection
    def setSelection(self, start, length):
        return self.v.setSelection(start, length)
    #@+node:ekr.20201227063438.130: *5* p.restore/saveCursorAndScroll
    def restoreCursorAndScroll(self):
        self.v.restoreCursorAndScroll()

    def saveCursorAndScroll(self):
        self.v.saveCursorAndScroll()
    #@+node:ekr.20201227063438.131: *4* p.setBodyString & setHeadString
    ...
    def setHeadString(self, s):
        ...
        p.v.initHeadString(s)
        p.setDirty()
    #@+node:ekr.20201227063438.132: *4* p.Visited bits
    #@+node:ekr.20201227063438.133: *5* p.clearVisitedInTree
    # Compatibility routine for scripts.

    def clearVisitedInTree(self):
        for p in self.self_and_subtree(copy=False):
            p.clearVisited()
    #@+node:ekr.20201227063438.134: *5* p.clearAllVisitedInTree
    def clearAllVisitedInTree(self):
        for p in self.self_and_subtree(copy=False):
            p.v.clearVisited()
            p.v.clearWriteBit()
    #@+node:ekr.20201227063438.135: *4* p.Dirty bits
    #@+node:ekr.20201227063438.136: *5* p.clearDirty
    def clearDirty(self):
        """(p) Set p.v dirty."""
        p = self
        p.v.clearDirty()
    #@+node:ekr.20201227063438.137: *5* p.inAtIgnoreRange
    def inAtIgnoreRange(self):
        ...
        for p in p.self_and_parents(copy=False):
            if p.isAtIgnoreNode():
                return True
        return False
    #@+node:ekr.20201227063438.138: *5* p.setAllAncestorAtFileNodesDirty
    ...
    #@+node:ekr.20201227063438.139: *5* p.setDirty
    def setDirty(self):
        """
        ...
        p.v.setAllAncestorAtFileNodesDirty()
        p.v.setDirty()
    #@+node:ekr.20201227063438.140: *3* p.Predicates
    #@+node:ekr.20201227063438.141: *4* p.is_at_all & is_at_all_tree
    def is_at_all(self):
    ...
    def in_at_all_tree(self):
        ...
        for p in p.self_and_parents(copy=False):
            if p.is_at_all():
                return True
        return False
    #@+node:ekr.20201227063438.142: *4* p.is_at_ignore & in_at_ignore_tree
...
#@+node:ekr.20201227063438.147: ** class VNode
#@@nobeautify
...
class VNode:
    ...
    #@+<< VNode constants >>
    #@+node:ekr.20201227063438.148: *3* << VNode constants >>
    ...
    #@-<< VNode constants >>
    #@+others
    #@+node:ekr.20201227063438.149: *3* v.Birth & death
    #@+node:ekr.20201227063438.150: *4* v.__init
    ...
    #@+node:ekr.20201227063438.151: *4* v.__repr__ & v.__str__
    def __repr__(self):
        return f"<VNode {self.gnx} {self.headString()}>"

    __str__ = __repr__
    #@+node:ekr.20201227063438.152: *4* v.dump
    def dumpLink(self, link):
        return link if link else "<none>"
    ...
    #@+node:ekr.20201227063438.153: *3* v.Comparisons
    #@+node:ekr.20201227063438.154: *4* v.findAtFileName
    def findAtFileName(self, names, h=''):
        ...
        if word in names and g.match_word(h, 0, word):
            ...
            return name
        return ""
    #@+node:ekr.20201227063438.155: *4* v.anyAtFileNodeName
    ...
    #@+node:ekr.20201227063438.156: *4* v.at...FileNodeName
    # These return the filename following @xxx, in v.headString.
    # Return the the empty string if v is not an @xxx node.

    def atAutoNodeName(self, h=None):
        return self.findAtFileName(self.atAutoNames, h=h)
    ...
    atNoSentFileNodeName = atNoSentinelsFileNodeName
    atAsisFileNodeName = atSilentFileNodeName
    #@+node:ekr.20201227063438.157: *4* v.isAtAllNode
    def isAtAllNode(self):
        """Returns True if the receiver contains @others in its body at the start of a line."""
        ###
        flag, i = g.is_special(self._bodyString, "@all")
        return flag
    #@+node:ekr.20201227063438.158: *4* v.isAnyAtFileNode
    ...
    #@+node:ekr.20201227063438.159: *4* v.isAt...FileNode
    ...
    isAtNoSentFileNode = isAtNoSentinelsFileNode
    isAtAsisFileNode = isAtSilentFileNode
    #@+node:ekr.20201227063438.160: *4* v.isAtIgnoreNode
    def isAtIgnoreNode(self):
        ...
        return flag
    #@+node:ekr.20201227063438.161: *4* v.isAtOthersNode
    def isAtOthersNode(self):
        """Returns True if the receiver contains @others in its body at the start of a line."""
        flag, i = g.is_special(self._bodyString, "@others")
        return flag
    #@+node:ekr.20201227063438.162: *4* v.matchHeadline
    ...
    #@+node:ekr.20201227063438.163: *3* v.copyTree
    ...
    def copyTree(self, copyMarked=False):
        ...
        **Important**: the v.parents ivar must be [] for all nodes.
        ...
        return v2
    #@+node:ekr.20201227063438.164: *3* v.Getters
    #@+node:ekr.20201227063438.165: *4* v.bodyString
    ...
    def bodyString(self):
        ...
        if not self.body_unicode_warning:
            ...
            g.error('not unicode:', repr(self._bodyString), self._headString)
        return g.toUnicode(self._bodyString)

    getBody = bodyString
        # Deprecated, but here for compatibility.
    #@+node:ekr.20201227063438.166: *4* v.Children
    #@+node:ekr.20201227063438.167: *5* v.firstChild
    def firstChild(self):
        v = self
        return v.children and v.children[0]
    #@+node:ekr.20201227063438.168: *5* v.hasChildren & hasFirstChild
    def hasChildren(self):
        v = self
        return len(v.children) > 0

    hasFirstChild = hasChildren
    #@+node:ekr.20201227063438.169: *5* v.lastChild
    def lastChild(self):
        v = self
        return v.children[-1] if v.children else None
    #@+node:ekr.20201227063438.170: *5* v.nthChild
    ...
    def nthChild(self, n):
        ...
        if 0 <= n < len(v.children):
            return v.children[n]
        return None
    #@+node:ekr.20201227063438.171: *5* v.numberOfChildren
    def numberOfChildren(self):
        v = self
        return len(v.children)
    #@+node:ekr.20201227063438.172: *4* v.directParents
    def directParents(self):
        ...
        v = self
        return v.parents
    #@+node:ekr.20201227063438.173: *4* v.hasBody
    def hasBody(self):
        """Return True if this VNode contains body text."""
        s = self._bodyString
        return s and len(s) > 0
    #@+node:ekr.20201227063438.174: *4* v.headString
    ...
    def headString(self):
        ...
        if not self.head_unicode_warning:
            ...
            g.error('not a string', repr(self._headString))
        return g.toUnicode(self._headString)
    #@+node:ekr.20201227063438.175: *4* v.isNthChildOf
    ...
    #@+node:ekr.20201227063438.176: *4* v.Status Bits
    #@+node:ekr.20201227063438.177: *5* v.isCloned
    def isCloned(self):
        return len(self.parents) > 1
    #@+node:ekr.20201227063438.178: *5* v.isDirty
    def isDirty(self):
        return (self.statusBits & self.dirtyBit) != 0
    #@+node:ekr.20201227063438.179: *5* v.isMarked
    def isMarked(self):
        return (self.statusBits & VNode.markedBit) != 0
    #@+node:ekr.20201227063438.180: *5* v.isOrphan
    def isOrphan(self):
        return (self.statusBits & VNode.orphanBit) != 0
    #@+node:ekr.20201227063438.181: *5* v.isSelected
    def isSelected(self):
        return (self.statusBits & VNode.selectedBit) != 0
    #@+node:ekr.20201227063438.182: *5* v.isTopBitSet
    def isTopBitSet(self):
        return (self.statusBits & self.topBit) != 0
    #@+node:ekr.20201227063438.183: *5* v.isVisited
    def isVisited(self):
        return (self.statusBits & VNode.visitedBit) != 0
    #@+node:ekr.20201227063438.184: *5* v.isWriteBit
    def isWriteBit(self):
        v = self
        return (v.statusBits & v.writeBit) != 0
    #@+node:ekr.20201227063438.185: *5* v.status
    def status(self):
        return self.statusBits
    #@+node:ekr.20201227063438.186: *3* v.Setters
    #@+node:ekr.20201227063438.187: *4*  v.Dirty bits
    #@+node:ekr.20201227063438.188: *5* v.clearDirty
    def clearDirty(self):
        """Clear the vnode dirty bit."""
        v = self
        v.statusBits &= ~v.dirtyBit
    #@+node:ekr.20201227063438.189: *5* v.setDirty
    def setDirty(self):
        """
    ...
    #@+node:ekr.20201227063438.190: *4*  v.Status bits
    #@+node:ekr.20201227063438.191: *5* v.clearClonedBit
    def clearClonedBit(self):
        self.statusBits &= ~self.clonedBit
    #@+node:ekr.20201227063438.192: *5* v.clearMarked
    def clearMarked(self):
        self.statusBits &= ~self.markedBit
    #@+node:ekr.20201227063438.193: *5* v.clearWriteBit
    def clearWriteBit(self):
        self.statusBits &= ~self.writeBit
    #@+node:ekr.20201227063438.194: *5* v.clearOrphan
    def clearOrphan(self):

        self.statusBits &= ~self.orphanBit
    #@+node:ekr.20201227063438.195: *5* v.clearVisited
    def clearVisited(self):
        self.statusBits &= ~self.visitedBit
    #@+node:ekr.20201227063438.196: *5* v.contract/expand/initExpandedBit/isExpanded
    def contract(self):
        """Contract the node."""
    ...
    #@+node:ekr.20201227063438.197: *5* v.initStatus
    def initStatus(self, status):
        self.statusBits = status
    #@+node:ekr.20201227063438.198: *5* v.setClonedBit & initClonedBit
    ...
    def initClonedBit(self, val):
        if val:
            ...
            self.statusBits &= ~self.clonedBit
    #@+node:ekr.20201227063438.199: *5* v.setMarked & initMarkedBit
    def setMarked(self):
        self.statusBits |= self.markedBit

    def initMarkedBit(self):
        self.statusBits |= self.markedBit
    #@+node:ekr.20201227063438.200: *5* v.setOrphan
    def setOrphan(self):
        """Set the vnode's orphan bit."""
        self.statusBits |= self.orphanBit
    #@+node:ekr.20201227063438.201: *5* v.setSelected
    # This only sets the selected bit.

    def setSelected(self):
        self.statusBits |= self.selectedBit
    #@+node:ekr.20201227063438.202: *5* v.setVisited
    # Compatibility routine for scripts

    def setVisited(self):
        self.statusBits |= self.visitedBit
    #@+node:ekr.20201227063438.203: *5* v.setWriteBit
    def setWriteBit(self):
        self.statusBits |= self.writeBit
    #@+node:ekr.20201227063438.205: *4* v.computeIcon & setIcon
    def computeIcon(self):
        val = 0; v = self
        if v.hasBody(): val += 1
        if v.isMarked(): val += 2
    ...
    def setIcon(self):
        pass  # Compatibility routine for old scripts
    #@+node:ekr.20201227063438.208: *4* v.saveCursorAndScroll
    def saveCursorAndScroll(self):
        ...
        try:
            ...
            pass
    #@+node:ekr.20201227063438.209: *4* v.setBodyString & v.setHeadString
    ...
    setTnodeText = setBodyString
    #@+node:ekr.20201227063438.210: *4* v.setSelection
    def setSelection(self, start, length):
        v = self
        v.selectionStart = start
        v.selectionLength = length
    #@+node:ekr.20201227063438.211: *3* v.setAllAncestorAtFileNodesDirty
    def setAllAncestorAtFileNodesDirty(self):
        """
        ...
        for v2 in v_and_parents(v):
            if v2.isAnyAtFileNode():
                v2.setDirty()
    #@+node:ekr.20201227063438.212: *3* v.Inserting & cloning
    ...
    def insertAsNthChild(self, n):
        ...
        return v2
    #@+node:ekr.20201227063438.213: *3* v.Low level methods
    #@+node:ekr.20201227063438.214: *4* v._addCopiedLink
    ...
    #@+node:ekr.20201227063438.215: *4* v._addLink & _addParentLinks
    ...
    #@+node:ekr.20201227063438.216: *5* v._addParentLinks
    def _addParentLinks(self, parent):
        ...
        if len(v.parents) == 1:
            for child in v.children:
                child._addParentLinks(parent=v)
    #@+node:ekr.20201227063438.217: *4* v._cutLink & _cutParentLinks
    ...
    #@+node:ekr.20201227063438.218: *5* v._cutParentLinks
    def _cutParentLinks(self, parent):
        ...
        if not v.parents:
            for child in v.children:
                child._cutParentLinks(parent=v)
    #@+node:ekr.20201227063438.219: *4* v._deleteAllChildren
    def _deleteAllChildren(self):
        ...
        for v2 in v.children:
            try:
                ...
                g.error(f"{v} not in parents of {v2}")
                g.trace('v2.parents:')
                g.printObj(v2.parents)
        v.children = []
    #@+node:ekr.20201227063438.220: *4* v._linkAsNthChild
    def _linkAsNthChild(self, parent_v, n):
        """Links self as the n'th child of VNode pv"""
        v = self  # The child node.
        v._addLink(n, parent_v)
    #@+node:ekr.20201227063438.221: *3* v.Properties
    #@+node:ekr.20201227063438.222: *4* v.b Property
    def __get_b(self):
        v = self
        return v.bodyString()
    ...
    #@+node:ekr.20201227063438.223: *4* v.h property
    def __get_h(self):
        v = self
        return v.headString()
    ...
    #@+node:ekr.20201227063438.224: *4* v.u Property
    def __get_u(self):
        v = self
    ...
    #@+node:ekr.20201227063438.225: *4* v.gnx Property
    def __get_gnx(self):
        v = self
        return v.fileIndex
